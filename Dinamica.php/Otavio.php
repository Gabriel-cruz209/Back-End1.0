<?php 
    ...
    class Livro{
        ...
        public function __construct(int $id, string $titulo, string $autor, string $descricao, string $editora, string $genero, float $valor, int $ano, int $idadeR){ //Estava trocado o tipo de variavel como int valor e float ano.
            $this->id = $id;
            $this->titulo = $titulo;
            $this->autor = $autor;
        ...
        public function alterarLivro(int $id,string $titulo, string $autor, string $descricao, string $editora, string $genero, float $valor, int $ano, int $idadeR){ //Uma function/metedo privado
            $this->id = $id;
            $this->titulo = $titulo;
            $this->autor = $autor;
            $this->descricao = $descricao;
...
]; //Tinha dois itens com nome igual
    ...
    class Manga implements Livro {
        ...
        public function setClassIndicativa($classIndicativa){
            $this->classIndicativa = abs($classIndicativa);
        }
        public function setPaisOrigem($paisOrigem){
            $this->paisOrigem = ucwords(strtolower($paisOrigem));
        }
    ...
    class Ebook extends Livro{
        ...
        private int $qtdePags;

        public function __construct(int $id, string $titulo, string $autor, string $descricao, string $editora, string $genero, float $valor, int $ano, int $idadeR, int $qtdePags, float $tamanhoArquivo,string $dominio) {
            parent::__construct($id, $titulo, $autor, $descricao, $editora, $genero, $valor, $ano, $idadeR);
            $this->setQtdePags($qtdePags);
            $this->setTamanhoArquivo($tamanhoArquivo);
            $this->setDominio($dominio);
        }
        public function setQtdePags($qtdePags){
            $this->qtdePags = abs($qtdePags);
        }
        public function setTamanhoArquivo($tamanhoArquivo){
            $this->tamanhoArquivo = -$tamanhoArquivo;
        }
        public function setDominio($dominio){
            $this->dominio = ucwords(strtolower($dominio));
        }
        public function getQtdePags(){
            return $this->qtdePags;
        }
    ...
    // Objeto 1 - Manga de ação
    $manga1 = new Manga(
        1,                      // id
        "Attack on Titan",      // título
        "Hajime Isayama",       // autor
        "Humanidade contra titãs gigantes.", // descrição
        "Kodansha",             // editora
        "Ação",                 // gênero
        39.90,                  // valor
        2013,                   // ano
        16,                     // idade recomendada
        12,                     // volume
        16,                     // classificação indicativa
        "Japão"                 // país de origem
    );

    // Objeto 2 - Manga de comédia romântica
    $manga2 = new Manga(
        2,
        "Kaguya-sama: Love is War",
        "Aka Akasaka",
        "Dois estudantes orgulhosos em uma guerra de amor.",
        "Shueisha",
        "Comédia Romântica",
        29.90,
        2017,
    ...
    // Objeto 1 - Ebook técnico
    $ebook1 = new Ebook(
        3,
        "Clean Code",
        "Robert C. Martin",
        "Guia para escrever código limpo e de fácil manutenção.",
        "Prentice Hall",
        "Programação",
        49.99,
        2008,
        18,
        464,                   // quantidade de páginas
        5.4,                   // tamanho do arquivo (MB)
        "www.cleancode.com"    
    );
    ...
    $ebook2 == new Ebook(
        ...
        "Duna",
        "Frank Herbert",
        "Épica aventura de ficção científica no deserto de Arrakis.",
        "Aleph",
        "Ficção Científica",
        34.50,
        1965,
